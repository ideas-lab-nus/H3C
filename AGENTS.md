...
- Project operators use the maintained runtime environment defined by their parent workspace contract. Other worktrees must not create their own `.venv`: set `PYTHONPATH=<target-worktree>\src`, invoke the maintained interpreter with `-m h3c.cli`, verify `h3c.__file__` resolves inside the target worktree, and require an identical `uv.lock`. External contributors may create a local environment using the installation instructions in `README.md`.
