...
## Registered numeric-consistency correction and recovery ruling

The fresh Official run `20260831T153137593047Z-7f4c89afb824` completed its first control hour and then stopped before hour two. Its deterministic consistency gate compared the four-decimal Agent-visible working-memory endpoint with the full-precision current physical state at a `1e-6` tolerance. For example, the same continuous temperature appeared as `24.5959` in memory and `24.5958606816597` in current state. This was a false conflict, not a physical discontinuity, Provider failure, or control decision failure.

The one numeric-precision owner now normalizes both sides to the same four-decimal Agent-visible representation before equality checking; materially different state still fails closed. Generic non-transport runtime exceptions now also publish an atomic `failure.json`. By the user's post-failure ruling, a mechanical failure is diagnosed and may be recovered automatically after minimal repair and changed-owner gates using a fresh run/test identity. Original failed runs remain immutable. Method changes, poor KPI, and ordinary model-contract degradation do not qualify for rerun. This publicly supersedes the campaign's one-attempt wording only for registered mechanical recovery; no resume or lucky method rerun is allowed.

## Registered current-view precision correction

The next fresh Official run `20260831T160140628580Z-147a82119d70` also completed its first control hour and then published an atomic terminal failure before hour two. The prior equality correction exposed a second, distinct duplicated precision owner: the Orchestrator current-zone projection had already rounded zone temperature to three decimals, whereas the working-memory endpoint used the shared four-decimal Agent-visible normalization. Thus the same physical value `24.5958606816597` was rendered as `24.596` and `24.5959` before comparison. The physical trajectory, timeline, Provider response and control method were not the cause.

The smallest correction routes current-zone temperature, PMV and setpoint through the existing shared Agent-visible numeric owner before both rendering and endpoint comparison. It removes the independent three-/two-decimal literals for these identity-bearing state fields; material disagreements still fail closed. The failed run and test remain immutable. A fresh recovery may start only after the changed-owner test, full offline gates, clean source commit, and release of the old PID, lock and test identity. No Prompt instruction, decision contract, control law, model setting, KPI or acceptance criterion changes.
