...
2. C8 correctly derived `program_direction=down` and a power-increasing effect,
   but the next-hour Agent view removed that deterministic result as audit-only;
3. a later non-weather recovery patch was rejected because the C1/C2 weather
...
These changes restore existing C1/C2/C8 and feedback semantics. They do not add
...
Provider/model, strict JSON schema, C0 `occupancy_routed` low thinking, one-hour
working memory, long-term memory off, P0, `pmv_step_c=0.3`, ProgramCheck,
C1/C2/C8, Budget, `S3 -> S2 -> S1`, reward formula, seven-day warm-up, 120-hour
...
- the historical bad threshold patch still produces C8 down/power-up evidence;
