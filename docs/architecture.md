...
2. One Executor per zone runs once per hour. All zone calls are issued
   ...
   settlement. Executor rationale follows the same nonempty, full-preservation,
...
Executor receives only its own previous-hour CAOL. An incomplete window is
...
zone Executor sees active experiences. Orchestrator never sees them, and
Executor references are audit-only. When disabled, the Prompt, schema, runtime
